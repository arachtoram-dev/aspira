...
async function toggleAccount(){if(!selectedUid)return;const d=users[selectedUid],a=d?.active!==false;if(!confirm(`Yakin ingin ${a?"menonaktifkan":"mengaktifkan"} akun ${d?.name||selectedUid}?`))return;try{showAspiraLoading(a?'Menonaktifkan akun...':'Mengaktifkan akun...');await update(ref(db,`users/${selectedUid}`),{active:!a,status:!a?"active":"inactive",updatedAtMs:Date.now()});showMessage(actionMessage,`Akun berhasil ${!a?"diaktifkan":"dinonaktifkan"}.`,"success");await renderUsers();hideAspiraLoading();setTimeout(closeAction,500);}catch(e){hideAspiraLoading();showMessage(actionMessage,"Perubahan status gagal. Periksa Firebase Rules.","error");}}
async function changeRole(){if(!selectedUid)return;const d=users[selectedUid],role=actionRoleSelect.value;if(selectedUid===auth.currentUser?.uid){showMessage(actionMessage,"Role akun yang sedang login tidak dapat diubah.","error");return;}if(d?.role&&!canManageRole("developer",d.role)||!canManageRole("developer",role)){showMessage(actionMessage,"Role tidak dapat diubah.","error");return;}if(!confirm(`Ubah role ${d?.name||selectedUid} menjadi ${role}?`))return;try{showAspiraLoading('Mengubah role akun...');await update(ref(db,`users/${selectedUid}`),{role,updatedAtMs:Date.now()});showMessage(actionMessage,"Role berhasil diubah.","success");await renderUsers();hideAspiraLoading();setTimeout(closeAction,500);}catch(e){hideAspiraLoading();showMessage(actionMessage,"Role gagal diubah.","error");}}
async function deleteProfile(){if(!selectedUid)return;const d=users[selectedUid];if(selectedUid===auth.currentUser?.uid){showMessage(actionMessage,"Akun yang sedang digunakan tidak dapat dihapus.","error");return;}if(!confirm(`Hapus data profil ${d?.name||selectedUid} dari Realtime Database?\n\nIni TIDAK menghapus akun Firebase Authentication.`))return;try{showAspiraLoading('Menghapus data akun...');await remove(ref(db,`users/${selectedUid}`));showMessage(actionMessage,"Data profil berhasil dihapus.","success");await renderUsers();hideAspiraLoading();setTimeout(closeAction,600);}catch(e){hideAspiraLoading();showMessage(actionMessage,"Data profil gagal dihapus.","error");}}
...
try{showAspiraLoading('Menyimpan perubahan profil...');await update(ref(db,`users/${uid}`),{name,email,role,updatedAtMs:Date.now()});showMessage(editMessage,"Perubahan berhasil disimpan.","success");
await renderUsers();
hideAspiraLoading();
setTimeout(()=>{editModal.hidden=true;},700);}catch(e){hideAspiraLoading();showMessage(editMessage,"Perubahan gagal disimpan.","error");}}
async function createAccount(){const name=newName.value.trim(),email=newEmail.value.trim(),password=newPassword.value,role=newRole.value;if(!name||!email||!password){showMessage(createMessage,"Nama, email, password, dan role wajib diisi.","error");return;}if(password.length<6){showMessage(createMessage,"Password minimal 6 karakter.","error");return;}createBtn.disabled=true;createBtn.textContent="Membuat akun...";try{showAspiraLoading('Membuat akun...');const c=await createUserWithEmailAndPassword(secondaryAuth,email,password),uid=c.user.uid;await update(ref(db,`users/${uid}`),{name,email,role,active:true,status:"active",createdAtMs:Date.now()});await signOut(secondaryAuth);newName.value="";newEmail.value="";newPassword.value="";showMessage(createMessage,`Akun ${name} berhasil dibuat. UID: ${uid}`,"success");await renderUsers();}catch(e){console.error(e);showMessage(createMessage,e.code==="auth/email-already-in-use"?"Email sudah terdaftar.":"Akun gagal dibuat.","error");}finally{hideAspiraLoading();createBtn.disabled=false;createBtn.textContent="+ Buat Akun";}}
