...
function render(){
  ...
  listEl.innerHTML=data.length?data.map(r=>`
    <article class="admin-report">
      ...
      </div><span class="status status-${String(r.status||"Diterima").toLowerCase().replace(/\s+/g,"-")}">${esc(r.status||"Diterima")}</span></div>
...
markNotificationsRead?.addEventListener("click",async()=>{
  ...
  const seen=Date.now();
  try{
    showAspiraLoading("Menandai notifikasi...");
...
async function markAllNotificationsRead(){
  ...
  try{
    showAspiraLoading("Menandai semua notifikasi...");
